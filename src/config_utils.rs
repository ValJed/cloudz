...
pub fn get_config(keys: [String; 2]) -> HashMap<String, String> {
    let env_file = File::open(".env").expect("You should provide en .env file with your config");

    let reader = BufReader::new(env_file);

    let regexes: HashMap<String, Regex> = keys.iter().fold(HashMap::new(), |mut acc, key| {
        let formatted_regex = format!(r"^{}", key);
        let regex = Regex::new(&formatted_regex).expect("Regex could not be generated");
